...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _hourlyScroller(futureModel currentWeatherModel) {
    if (today == true) {
      return HourlyScroller(
          weatherModel: currentWeatherModel.forecast.forecastday[0]);
    } else {
      return HourlyScroller(
          weatherModel: currentWeatherModel.forecast.forecastday[1]);
    }
  }
  ...
  Widget _getBottomHalf({required futureModel weatherModel}) {
    if (today == true) {
      return bottomHalf(
        weatherModel: weatherModel.forecast.forecastday[0],
        hournow: DateTime.now().hour,
        textstyle: const TextStyle(
            fontFamily: 'Comfortaa', fontSize: 20, color: Colors.white),
        alerts: currentWeatherModel.alerts,
      );
    } else {
      return bottomHalf(
        weatherModel: weatherModel.forecast.forecastday[1],
        textstyle: const TextStyle(
            fontFamily: 'Comfortaa', fontSize: 20, color: Colors.white),
        hournow: 0,
        alerts: currentWeatherModel.alerts,
  ...
  String _getSunriseTime({required futureModel weatherModel}) {
    if (today == true) {
      String time =
          to24Hour(weatherModel.forecast.forecastday[0].astro.sunrise);
      return time;
    } else {
      String time =
          to24Hour(weatherModel.forecast.forecastday[1].astro.sunrise);
  ...
  String _getCurrentTemp() {
    return currentWeatherModel
        .forecast.forecastday[0].hour[DateTime.now().hour].tempC
        .round()
        .toString();
  }
      ...
      return Row(
        children: [
          ...
          Expanded(
              ...
              child: Center(
                  child: WhiteText(
                      text: DateFormat("EEEEE dd/MM/yyyy")
                          .format(
                              currentWeatherModel.forecast.forecastday[0].date)
                          .toString()))),
          Expanded(
            flex: 1,
          ...
          Expanded(
              ...
              child: Center(
                  child: WhiteText(
                      text: DateFormat("EEEEE dd/MM/yyyy")
                          .format(
                              currentWeatherModel.forecast.forecastday[1].date)
                          .toString()))),
          const Expanded(flex: 1, child: FittedBox()),
        ],
      );
    }
  }
            ...
            children: [
                      ...
                      child: Column(
                        children: [
                          ...
                          Align(
                              alignment: Alignment.centerRight,
                              child: WhiteText(
                                text: currentWeatherModel.location.name,
                              )),
                        ],
                      ),
                    ),
              ...
              Row(
                children: [
                      ...
                      child: Column(
                        children: [
                          Align(
                            alignment: Alignment.centerLeft,
                            child: WhiteText(
                                text: "${_getCurrentTemp()}°", size: 30),
                          ),
                          Align(
                              alignment: Alignment.centerLeft,
                              child: WhiteText(
                                  text: currentWeatherModel.current.condition
                                      .toString(),
                                  size: 25))
                        ],
                      ),
                  ...
                  CurrentWeatherIcon(
                    iconCode: currentWeatherModel.current.condition.code,
                  ),
                ],
              ),
              ...
              Padding(
                  ...
                  child: SizedBox(
                      width: MediaQuery.of(context).size.width,
                      height: 100,
                      child: SunriseSunsetRectangle(
                          sunrisetime: _getSunriseTime(
                              weatherModel: currentWeatherModel),
                          sunsettime:
                              _getSunsetTime(weatherModel: currentWeatherModel),
                          textstyle: const TextStyle(
